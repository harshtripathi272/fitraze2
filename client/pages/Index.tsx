...
export default function Index({ user, onLogout }: IndexProps) {
  ...
  // Macro distribution data for donut chart - Updated with new colors
  const macroData = [
    {
      name: "Protein",
      value: Math.round(
        ((dailyStats.protein.current * 4) / dailyStats.calories.current) * 100,
      ),
    ...
    }, // Sky Blue
    {
      name: "Carbs",
      value: Math.round(
        ((dailyStats.carbs.current * 4) / dailyStats.calories.current) * 100,
      ),
    ...
    }, // Amber
    {
      name: "Fat",
      value: Math.round(
        ((dailyStats.fat.current * 9) / dailyStats.calories.current) * 100,
      ),
  ...
  const handleFoodAdded = (food: Food, mealType: string) => {
    ...
    setLoggedFoods((prev) => [
      ...prev,
      { food, mealType, timestamp: new Date() },
    ]);
    ...
    setDailyStats((prev) => ({
      ...
      calories: {
        ...prev.calories,
        current: Math.min(
          prev.calories.current + food.calories,
          prev.calories.goal + 500,
        ),
      },
      protein: {
        ...prev.protein,
        current: Math.min(
          prev.protein.current + food.protein,
          prev.protein.goal + 50,
        ),
      },
      carbs: {
        ...prev.carbs,
        current: Math.min(
          prev.carbs.current + food.carbs,
          prev.carbs.goal + 100,
        ),
      },
      fat: {
        ...prev.fat,
        current: Math.min(prev.fat.current + food.fat, prev.fat.goal + 30),
      },
    }));
  };
  ...
  const handleWorkoutCompleted = (workout: any) => {
    setWorkoutStats({
      completedToday: true,
      exercisesCompleted: workout.completedCount,
    });
  };

  const handleWaterAdded = (amount: number, isIce?: boolean) => {
    setDailyStats((prev) => ({
      ...prev,
      water: {
        ...prev.water,
  ...
  const calorieProgress =
    (dailyStats.calories.current / dailyStats.calories.goal) * 100;
  const proteinProgress =
    (dailyStats.protein.current / dailyStats.protein.goal) * 100;
  const waterProgress =
    (dailyStats.water.current / dailyStats.water.goal) * 100;

  // Enhanced tooltip for macro donut chart with precise values
      ...
      return (
        <div className="glass-card p-3 border border-glass-border rounded-lg text-xs glow-accent animate-slide-in-up">
          ...
          <p className="text-foreground font-medium">
            {grams}g • {data.calories} calories
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-background/80 px-3 sm:px-4">
      ...
      {/* Calorie Goal - Moved to separate section */}
      <div className="px-1 sm:px-4 mt-6">
        <div className="glass-card p-4 sm:p-6 rounded-2xl border border-glass-border glow-intense text-center">
          ...
          <ProgressRing
            ...
            size={90}
            className="sm:scale-110 glow-intense"
          >
      ...
      <div className="mb-6">
        <h3 className="text-base sm:text-lg font-semibold mb-4 px-1 sm:px-4">
          Today's Insights
        </h3>
        <div className="overflow-x-auto scrollbar-hide">
          <div
            className="flex space-x-4 px-1 sm:px-4"
        ...
        <div className="glass-card p-4 sm:p-6 rounded-2xl border border-glass-border glow">
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-xs sm:text-sm font-medium">
                  {dailyStats.protein.current}g / {dailyStats.protein.goal}g
                </div>
                <div className="w-20 sm:w-24 h-2 bg-muted rounded-full overflow-hidden">
                  <div
                    className="h-full bg-accent progress-fill glow-accent"
                    style={
                      {
                        "--progress-width": `${Math.min(proteinProgress, 100)}%`,
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-xs sm:text-sm font-medium">
                  {dailyStats.carbs.current}g / {dailyStats.carbs.goal}g
                </div>
                <div className="w-20 sm:w-24 h-2 bg-muted rounded-full overflow-hidden">
                  <div
                    ...
                    style={
                      {
                        "--progress-width": `${Math.min((dailyStats.carbs.current / dailyStats.carbs.goal) * 100, 100)}%`,
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-xs sm:text-sm font-medium">
                  {dailyStats.fat.current}g / {dailyStats.fat.goal}g
                </div>
                <div className="w-20 sm:w-24 h-2 bg-muted rounded-full overflow-hidden">
                  <div
                    ...
                    style={
                      {
                        "--progress-width": `${Math.min((dailyStats.fat.current / dailyStats.fat.goal) * 100, 100)}%`,
      ...
      {/* Enhanced Today's Activities with Slidable Tabs */}
      ...
      <div className="px-1 sm:px-4 mb-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button
            className="glass-enhanced h-14 sm:h-16 flex-col space-y-1 glow-intense hover:glow-intense btn-glow-effect ripple-effect animate-slide-from-left"
            variant="outline"
          ...
          <Button
            className="glass-enhanced h-14 sm:h-16 flex-col space-y-1 glow-accent hover:glow-accent btn-glow-effect ripple-effect animate-slide-from-right"
            variant="outline"
          ...
          <Button
            className="glass-enhanced h-14 sm:h-16 flex-col space-y-1 glow hover:glow-intense btn-glow-effect ripple-effect animate-slide-from-left"
            variant="outline"
      ...
      <FoodLogDialog
        ...
        onScanMeal={handleScanMeal}
        onNavigateToCustomFood={() => navigate("/quick-add-food")}
      />
      ...
      <StreakCelebration
        open={showLaunchCelebration}
        onOpenChange={setShowLaunchCelebration}
        streakData={streakStats}
        isLaunchCelebration={true}
      />
    </div>
  );
}
