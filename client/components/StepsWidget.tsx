...
export function StepsWidget({ className }: StepsWidgetProps) {
  ...
  const progress = stepData ? (stepData.steps / stepData.goal) * 100 : 0;
        ...
        {!isConnected || !stepData ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <ProgressRing progress={progress} size={90} color="accent" className="glow-accent">
                <div className="text-center">
                  <div className="text-lg font-bold text-accent">{stepData.steps.toLocaleString()}</div>
              ...
              <div className="flex-1 ml-4 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{stepData.goal.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Remaining:</span>
                  <span className="font-medium text-primary">
                    {Math.max(stepData.goal - stepData.steps, 0).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
            ...
            <div className="relative">
              <div className="h-8 bg-muted/20 rounded-full overflow-hidden relative">
                ...
                <div
                  ...
                  style={{ left: getRunnerPosition(), transform: "translateY(-50%) translateX(-50%)" }}
                >
              ...
              <div className="flex justify-between text-xs text-muted-foreground mt-2">
                <span>0</span>
                <span>{(stepData.goal / 2).toLocaleString()}</span>
                <span>{stepData.goal.toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center">
                ...
                <div className="text-sm font-bold">{stepData.calories}</div>
              ...
              <div className="text-center">
                ...
                <div className="text-sm font-bold">{stepData.distance}km</div>
              ...
              <div className="text-center">
                ...
                <div className="text-sm font-bold">{stepData.activeMinutes}m</div>
            ...
            {progress >= 100 && (
              <div className="glass-card p-3 rounded-lg border border-glass-border glow animate-scale-in">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium text-green-400">Goal Achieved! 🎉</span>
                </div>
            ...
            <div className="flex flex-wrap gap-2">
              {stepData.steps >= 5000 && (
                <Badge variant="outline" className="glass text-xs animate-scale-in">🚶 5K Steps</Badge>
              )}
              {stepData.steps >= 10000 && (
                <Badge variant="outline" className="glass text-xs animate-scale-in">🏃 10K Steps</Badge>
              )}
              {stepData.floors >= 10 && (
                <Badge variant="outline" className="glass text-xs animate-scale-in">🏢 Climber</Badge>
              )}
              {stepData.activeMinutes >= 60 && (
                <Badge variant="outline" className="glass text-xs animate-scale-in">⏰ Active Hour</Badge>
              )}
            </div>
          </div>
        )}
